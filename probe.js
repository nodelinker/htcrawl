...
function initProbe(options, inputValues) {
	...
	Probe.prototype.filterMutation = function () {
		var firstDOMMutation = [];
		var isExist = false
	...
	*/
		...
		*/
		...
		var pdh = function (e) {
			...
			*/
			...
			e.stopPropagation(); // 停止事件在 DOM 树中的传播，防止事件继续向上或向下传递
			e.stopImmediatePropagation(); // 停止事件在 DOM 树中的传播，防止事件继续向上或向下传递；并阻止执行后续绑定在同一元素上的事件处理程序
		}
	...
	Probe.prototype.getTrigger = function () {
		...
		return {
			element: this.describeElement(this.curElement.element),  // 生成一个唯一的 CSS 选择器
	...
	Probe.prototype.getElementSelector = function (element) {
		// 生成一个唯一的 CSS 选择器，以便在页面中定位和操作该元素
	...
	Probe.prototype.stringToLowerCase = function (str) {
	...
	Probe.prototype.getXpathSelector = function (element) {
		// 获取DOM的Xpath
		let xpath = ""
